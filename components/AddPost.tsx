...
const Addpost: FC<SearchProps> = ({ onBack }) => {
  const [details,setdetails]=useState({})
  useEffect(()=>{
  fetch('http://localhost:5000/userProfile').then((res) => res.json())
    .then((updatedProfile) => {
...
console.log(details)
  ...
  const [fileType, setFileType] = useState<string | null>(null); // Track file type (image/video)
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      const fileExtension = file.type.split("/")[0]; // Get file type (image/video)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.append("profile",details.avatar)
  ...
  return (
    <div className="max-w-xl mx-auto p-4">
      <div className="bg-white/60 backdrop-blur-lg rounded-xl border border-green-100 shadow-sm p-4">
        ...
        <form onSubmit={(e) => { handleSubmit(e); onBack(); }} className="space-y-4">
          <div className="flex gap-4 mb-4">
            <button
              type="button"
              onClick={() => setIsStory(false)}
              className={`flex-1 py-2 rounded-full ${
                !isStory ? "bg-gradient-to-r from-green-500 to-blue-600 text-white" : "bg-gray-100"
              }`}
            >
              Post
            </button>
          ...
          {!preview ? (
            ...
            <div className="relative">
              {fileType === "image" ? (
                ...
                <video src={preview} controls className="w-full rounded-xl max-h-96" />
              )}
              <button
                type="button"
                onClick={() => {
          ...
          <button
            type="submit"
            className="w-full py-2 rounded-full bg-gradient-to-r from-green-500 to-blue-600 text-white font-medium hover:shadow-lg transition-all duration-300"
          >
            Share
          </button>
        </form>
